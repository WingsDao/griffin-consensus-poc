...
exports.toTxObject = function toTxObject(serializedTx) {
    ...
    const msgHash = keccak256(Buffer.from(serializedTx.slice(2), 'hex'));
    const V       = parseInt(v, 16);
    const R       = Buffer.from(r.slice(2), 'hex');
    const S       = Buffer.from(s.slice(2), 'hex');

    const publicKey = ethereumUtil.ecrecover(msgHash, V, R, S);

    const from = '0x' + keccak256(publicKey).slice(12).toString('hex');

    // FIXME NOTE recovered public key is incorrect.

    // console.log('from publicKey', from, '0x' + publicKey.toString('hex'));
    // console.log('msg hash', '0x' + msgHash.toString('hex'));
