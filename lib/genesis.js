 ...
 *
 * @param   {String} path Path to genesis block.
 * @returns {Object}      Genesis block.
 */
Genesis.loadFromFile = function loadFromFile(path) {
    let genesisData;
    const genesisJson = fs.readFileSync(path);

    try {
        genesisData = JSON.parse(genesisJson);
    } catch (e) {
        throw `Can't load genesis from file: ${e.message}`;
    }

    const genesis = Genesis();

    Object.assign(genesis, genesisData);

    return genesis;
};

/**
 * Process genesis block allocation data and generate initial state.
 *
 * @returns {Object} Genesis block with state.
 */
Genesis.prototype.getBlock = function getBlock() {
    const state = [];

    for (const allocObject of this.alloc) {
    ...
    const block = Object.assign(this, {
        state
    });
